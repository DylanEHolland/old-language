...
namespace liz::frontend {
    ...
    struct node *parse(std::vector<std::string> tokens) {
        ...
        for(int index = 0; index < tokenCount; index++) {
            ...
            if(token == "{") {
                std::cout << "Found block open" << std::endl;
                ...
                if(index < tokenCount - 1) {
                    ...
                    std::cout << "-> Finished Recursing " << std::endl;
                ...
                std::cout << "Found struct declaration" << std::endl;
                self->type = STRUCT;
            } else {
                std::cout << "Found value: " << token << std::endl;
                self->type = VALUE;
                self->value = token;
            }
    ...
    struct node *reverseParsedTree(struct node *code_tree) {
        ...
        while(initialTreeToken->active) {
            ...
            // for some reason these are already reversed :/
            // if(token->child) {
            //     token->child = reverseParsedTree(token->child);
            // }
